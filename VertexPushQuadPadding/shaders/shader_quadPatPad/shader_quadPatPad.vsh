...
void main()
{
  ...
  vec4 positionOriginal = vec4(in_Position, 1.0);
  vec4 positionModified = positionOriginal;
  vec2 paddingDirection = (corner * 2.0 - 1.0);
  positionModified.xy += (paddingDirection * VSH_vertexPadding);
  
  
  // Set the clip-space position with padded position.
  gl_Position = gm_Matrices[MATRIX_WORLD_VIEW_PROJECTION] * positionModified;
  ...
  vCorner = corner;
  vPositionOriginal = positionOriginal.xy;
  vPositionModified = positionModified.xy;
}
