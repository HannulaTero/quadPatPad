...
#define MAX_NEIGHBOURS 16 // Match with GML function.
...
{
  return (length(color) > FSH_threshold);
}
...
void main()
{
  ...
  float dxCoord = dFdx(vCoord.x) / dFdx(vSpacePosOriginal.x);
  float dyCoord = dFdy(vCoord.y) / dFdy(vSpacePosOriginal.y);
  
  float dxCorner = dFdx(vCorner.x) / dFdx(vSpacePosOriginal.x);
  float dyCorner = dFdy(vCorner.y) / dFdy(vSpacePosOriginal.y);
  
  
  // Differences between interpolated values of padded and original positions.
  vec2 spacePosDiff = (vSpacePosModified.xy - vSpacePosOriginal.xy);
  

  // Extrapolate the true, un-stretched coordinates
  vec2 trueCoord = vec2(
    vCoord.x + (spacePosDiff.x * dxCoord),
    vCoord.y + (spacePosDiff.y * dyCoord)
  );

  vec2 trueCorner = vec2(
    vCorner.x + (spacePosDiff.x * dxCorner),
    vCorner.y + (spacePosDiff.y * dyCorner)
  );
  ...
  vec2 texel  = abs(vec2(dxCoord, dyCoord));
  vec2 corner = abs(vec2(dxCorner, dyCorner));
